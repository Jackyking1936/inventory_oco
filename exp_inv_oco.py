...
from PySide6.QtCore import QMutex, Qt, QObject, Signal
...
class Communicate(QObject):
	...
	# 定义一个带参数的信号
	signal_with_arg = Signal(int, int, str)
...
class MainApp(QWidget):
	def __init__(self):
		...
		self.mutex = QMutex()
		self.c = Communicate()
		self.c.signal_with_arg.connect(self.with_arg_function)
		self.fake_price = 0
		...
		self.asdf = 10
		self.timer = RepeatTimer(1, self.handle_price)
		self.timer.start()
	...
	def handle_price(self):
		self.asdf+=1
		print(self.asdf)
		self.c.signal_with_arg.emit(3, 3, str(self.asdf))
		# self.tablewidget.item(3, 3).setText(str(self.asdf))

	def with_arg_function(self, row, col, value):
		...
		if content.buy_sell == BSAction.Sell:
			...
			if remain_qty == 0:
				if content.user_def == "inv_SL":
					...
					self.print_log("手動出場 "+content.stock_no+": "+content.filled_qty+"股, 成交價:"+str(content.filled_price))
					self.stop_loss_dict.pop(content.stock_no)
					self.take_profit_dict.pop(content.stock_no)
					# stop monitor and unsubscribe
					self.stop_loss_dict.pop(content.stock_no)
